...
function dfm_read ($dfm_file_name, $aform = false, $str = false, $form_name = false, $is_runtime = false)
{
	...
	if ((!$str) && is_readable($dfm_file_name))
	...
	for ($i=0, $c = sizeof($components); $i < $c; $i++)
	{
		$el =& $components[$i];
...
function loadFormEvents(TForm &$form)
{
	...
	$events = array('onClick','onClose','onCloseQuery','onDblClick','onKeyUp','onKeyPress','onKeyDown',
		...
		'onUndock','onStartDock','onEndDock',
					'OnDuringSizeMove','OnStartSizeMove','OnEndSizeMove','OnPopup', 'onMouseWheel', 'onMouseWheelUp', 'onMouseWheelDown');
        
	for ($i=0,$c=count($objs_l);$i<$c;$i++) {
		$self = $objs_l[$i];
		$o_name = component_name($self);
		
		for ($j=0,$c2=count($events);$j<$c2;$j++) {
			$ev = $events[$j];
