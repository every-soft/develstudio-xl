...
class styleMenu 
{
	...
	static function param($name, $value = false)
	{
		...
		if ($value)
			stylemenu_command(0, $name, $value);
		else
...
function menuItem ($caption, $styled = false, $name = '', $onClick = '', $sc = false, $img = false)
{
	...
	if ($img)
	{
		if (is_file (replaceSl($img)))
			$result->picture->loadFromFile(replaceSr($img));
		else
			if (is_file (replaceSl(DOC_ROOT.'/'.$img)))	
...
class TMenuItem extends TControl 
{
	...
	function set_break ($v)
	{
		$this->set_prop('break', $v);
	}
	
	function get_break ()
	{
		return $this->get_prop('break');
	}
...
class TPopupMenu extends TControl 
{
	...
	function get_items()
	{
		...
		for ($i = 0, $c = (popup_item_count($this->self)-1); $i < $c; $i++)
		{
			$result[] = _c(popup_item_id($this->self, $i)); // f. b.
		}
