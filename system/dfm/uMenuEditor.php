...
class menuEditor 
{
    ...
    static function getParam($index)
	{
        if ($index >= 0)
		{
			...
			$return['autoCheck']  = trim($params[4]);
			$return['default']  = trim($params[5]);
			$return['enabled']  = trim($params[6]);
			$return['checked']  = trim($params[7]);
			
			if (trim($params[8])) {
				$return['break']  = trim($params[8]);
			} else {
				$return['break']  = 'mbNone';
			}
            ...
            return array('text' => '', 'func' => '', 'icon' => '', 'scut' => '', 'name' => '', 'autoCheck' => false, 'default' => false, 'enabled' => true, 'checked' => false, 'break' => 'mbNone');
		}
    }
    ...
    static function setParam($index, $params)
	{
        ...
        $item .= $params['func'] .','. $params['icon'] .','. $params['scut'] .','. $params['name'] .','. $params['autoCheck'] .','. $params['default'] .','. $params['enabled'] .','.$params['checked'] .','. $params['break'];
        $item .= ']';

        $result[$index] = $item;
        c('edt_menuEditor')->result = join (_BR_, $result);
    }
    ...
    static function getParams()
	{
        ...
        $result['autoCheck'] = val('edt_menuEditor->c_autoCheck');
		$result['default'] = val('edt_menuEditor->c_default');
		$result['enabled'] = ! val('edt_menuEditor->c_enabled'); // fix bug
		$result['checked'] = val('edt_menuEditor->c_checked');
		$result['break'] = (string) c('edt_menuEditor->c_break')->items->selected;
    ...
    static function setParams($result)
	{
        ...
        val('edt_menuEditor->c_autoCheck', $result['autoCheck']);
        val('edt_menuEditor->c_default', $result['default']);
        val('edt_menuEditor->c_enabled', ! $result['enabled']); // fix bug
        val('edt_menuEditor->c_checked', $result['checked']);
		c('edt_menuEditor->c_break')->items->selected = $result['break'];
	...
	static function deleteIconFile ()
	{
		global $projectFile;
		
		$prDirectory = dirname ($projectFile);
		$dtDirectory = $prDirectory . '/data';
		$file = c('edt_menuEditor->e_icon')->text;
		
		if (!trim($file)) return;

		$lastFile = $prDirectory . '/' . $file;
		if (substr($file, 0, 6) == '{res}/')
			file_delete ($dtDirectory . '/' . substr($file, 6));
		
		file_delete ($lastFile);
	}
}
...
class ev_edt_menuEditor_it_delete {
    static function onClick($self){ ev_edt_menuEditor_btn_delete::onClick($self); }
}

class ev_edt_menuEditor_it_insert {
    static function onClick($self){ ev_edt_menuEditor_btn_insert::onClick($self); }
}

class ev_edt_menuEditor_it_up {
    static function onClick($self){ ev_edt_menuEditor_btn_up::onClick($self); }
}

class ev_edt_menuEditor_it_down {
    static function onClick($self){ ev_edt_menuEditor_btn_down::onClick($self); }
}
...
class ev_edt_MenuEditor_btn_pathToIcon 
{
	...
	static function onClick ($self)
	{
		...
		$dlg->options = 'ofNoLongNames,ofFileMustExist,ofNoValidate';
		...
		{		
			...
			if ($directory != $prDirectory && $directory != $dtDirectory)
			{
				file_copy ($file, $dtDirectory . '/' . basename($file));
				
			}

			c('edt_menuEditor->e_icon')->text = '{res}/' . basename($file);
