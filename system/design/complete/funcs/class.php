...
class complete_Funcs {
    ...
    function initF(){
		...
		self::$sourceFiles[] = $engineDir . '/main/vseditor.php';
		self::$sourceFiles[] = $engineDir . '/main/catbuttons.php';
    ...
    function getScriptsFunctions($sort = false)
	{
		...
		/* автоподключение .inc файлов */
		if(!empty($myProject->config['modules'])) // fix bug
			foreach ($myProject->config['modules'] as $mod)
			{
				$incFile = DOC_ROOT . '../ext/' . basenameNoExt($mod) . '.inc';
        ...
        foreach($files as $file)
		{
			...
			if(!empty($info['functions'])) // fix err
    ...
    function init()
	{
		...
		if (file_exists(DOC_ROOT . '/design/complete/consts.php')){
            $consts = include DOC_ROOT . '/design/complete/consts.php';
    ...
    function getList($lineText)
	{
        ...
        foreach ($funcs as $i => $info)
		{
            ...
            $text = str_replace('nubmer ', '[$r]number[$b] ',$text); // nubmer ???
