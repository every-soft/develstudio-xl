...
class PHPSyntax {
    ...
    public function checkObjects ($code, $org_code)
	{
        ...
        foreach ((array)$arr[1] as $obj_name)
		{
            ...
            if (in_array($obj_name, $check_objs)) 
				continue;
    ...
    static function analiz ($code, $doc_root = false)
	{
        ...
        foreach ($syn->tokens as $i => $token)
		{
			...
			{ 
                ...
                if (in_array($id, array (T_PUBLIC, T_PROTECTED, T_PRIVATE, T_STATIC, T_VAR))) {
					if ($id == T_VAR)
						$last_sec = T_PUBLIC;
					else
						$last_sec = $id;        
				...
				{
                    if ($syn->tokens[$i-4][0] == T_COMMENT || $syn->tokens[$i-4][0] == T_DOC_COMMENT)
                    ...
                    for($k = $i; $k < $c; $k++)
					{
						...
						} elseif (isset($syn->tokens[$k][1]))
						{
							if (strtolower ($syn->tokens[$k][1]) == 'null')
								$defaults[count($defaults)-1] = $syn->tokens[$k][1];
						}
                    ...
                    if ($n_class)
					{
                        ...
                        if (in_array ($syn->tokens[$i-4][1], array('static', 'private', 'public', 'protected')))
						...
						if (isset($syn->tokens[$i-6][1]))
							if (!in_array ($syn->tokens[$i-6][1], array('protected', 'private')))
                ...
                if ($id == 309 && in_array($syn->tokens[$i-2][0], array(T_PUBLIC, T_STATIC, 347, T_VAR))
								&& !in_array($syn->tokens[$i-4][1], array('protected', 'private')))
				{

                    if (count($result['classes'][$n_class]['methods']) == 0)
					{
                        if (($syn->tokens[$i-2][0] == T_PUBLIC) /*|| ($syn->tokens[$i-2][0] == T_STATIC)*/ ||($syn->tokens[$i-2][0] == T_VAR))
						{
				...
				{
					if (isset($result['classes'][$n_class]['methods']))
                    if (count($result['classes'][$n_class]['methods']) == 0){
