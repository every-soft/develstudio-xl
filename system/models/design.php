...
function searchList ($needle) 
{
	...
	{
		if (strpos(strtolower(' '.$c['CAPTION'] .' - '. $c['CLASS']), $needle) ||
			strpos(strtolower(' '.$c['GROUP']), $needle) ||
			strpos(strtolower(' '.t('gr_'.$c['GROUP'])), $needle)) 
		{
...
class myDesign 
{
    ...
    static function changeName($obj, $new_name)
	{
        ...
        if ($fmEdit->findComponent ($new_name)) 
			return;
	...
	{ 
        ...
        if ($gen)
		{
            ...
            $myProperties->generate($obj->self,c('fmMain->tabProps'));
    ...
    static function createComponent($x = 20, $y = 20, $parent = nil, $w = false, $h = false)
	{
        ...
        if ($c)
		{
            if ($c['IS_ONE'] && self::countComponentsByClass($c['CLASS'])>0){
                MessageBox(t('This object class %s must be one on form', $c['CLASS']), '', MB_ICONWARNING);
            ...
            if ($class == 'TPageControl' || $class == 'TTabControl')
			{
    ...
    static function _selectComponent($self, $target, $x, $y)
	{
        ...
        if ($selectedClass && self::isWinControl($obj))
		{
            ...
            if ($selectedClass)
			{
                ...
                $myProperties->generate($target, c('fmPropsAndEvents->tabProps'));
                $myEvents->generate($obj);
        ...
        global $_designSel, $fmEdit, $_startAY, $_startAX, $isMouseDown;
            ...
            if ($isMouseDown && abs($_startX-$x) > 10 && abs($_startY-$y) > 10){
                ...
                if (!$_designSel){
                    ...
                    $_designSel->AlphaBlendValue = 150;
    ...
    static function mouseUp($self, $button, $shift, $x, $y)
	{
        ...
        if ($selectedClass) 
		{
            ...
            foreach ($components as $el){
                ...
                if (!$el->name || in_array(rtii_class($el->self), array('TEvents', 'TSizeCtrl'))) 
					continue;
    ...
    static function formProps($clear = false)
	{
        ...
        $myProperties->generate($fmEdit->self, c('fmPropsAndEvents->tabProps'));
	...
	{       
        if ( c('fmMain->tmpEdit')->focused || c('fmComponents')->focused || c('fmComponents->list')->focused || c('fmPropsAndEvents->eventList')->focused || c('fmPropsAndEvents->tabEvents')->focused )
            return true;
    ...
    static function editorPopup()
	{
        ...
        c('fmMain->itemLock')->checked  = ($selObj->tag == 2012);
        с('fmMain->itemGroup')->caption = count(self::getGroup($selObj)) > 0 ?
                                            t('Ungroup') : t('Group');
    }
    ...
    static function keyPaste()
	{
		...
		if(!empty($objs))
			foreach ($objs as $el) {
				...
				if (method_exists($el['cmp'], '__updateDesign')) 
					$el['cmp']->__updateDesign();
				if (method_exists($el['cmp'], '__pasteDesign')) 
					$el['cmp']->__pasteDesign();
				...
				if(!empty($el['childs'])) {
					foreach ($el['childs'] as $x => $child) {
						...
						if (method_exists($child, '__updateDesign')) 
							$child->__updateDesign();
						if (method_exists($child, '__pasteDesign')) 
							$child->__pasteDesign();
    ...
    static function szRefresh()
	{
        ...
        lockWindowUpdate(c('fmPropsAndEvents->tabProps')->handle);
        myUtils::saveForm();
        ...
        $myEvents->generate($el);
        $myProperties->generate($el->self, c('fmPropsAndEvents->tabProps'));
        lockWindowUpdate(0);
    }
    ...
    static function findComponent()
	{
        myVars::set(0, 'popupShow');
        
        global $_sc, $fmEdit;
		
		$name = inputText(t('Find component'), t('Component name'), '', false);
		
		if ($name != mrCancel)
        if ($name = trim ($name))
		{
			if (is_object ($fmEdit->findComponent($name)))
			{
				$_sc->clearTargets();
				$_sc->addTarget ($fmEdit->findComponent($name));
			} 
			else {
				MessageBox(t('Component not found!'), '', MB_ICONERROR);
			}
		}
    ...
    static function resizeEditForm($self)
	{
        ...
        c('fmMain->shapeSize')->w = $fmEdit->w + 17;
        c('fmMain->shapeSize')->h = $fmEdit->h + 17; 
    }
