...
class myActions {
    ...
    static function createDialog($code, $action, &$params)
	{
        ...
        $form = new TForm;
        $form->tag = 2012;
        $form->caption = $action['TEXT'];
        $form->name = str_replace('.', '_', $code) . '_action';
        ...
        $btn->w = 100;
        $btn->h = 25;
        $btn->x = $form->w - $btn->w - 10 - $gb->x*3;
        
        $btn->modalResult = mrOk;
        $btn->caption = t('ok');
        
        // создаем кнопку "Отмена"
        $btn2 = new TBitBtn($form);
        $btn2->parent = $gb;
        $btn2->w = $btn->w;
        $btn2->h = $btn->h;
        $btn2->x = $form->w - $btn->w - 15 - $btn2->w - $gb->x*3;
        ...
        $btn->y  = $form->h - $btn->h * 2 - 15 - $gb->y*2;
        $btn2->y = $btn->y;
...
class action_Simple 
{
    ...
    function insertCommand($command)
	{
        ...
        $lineY = c('fmPHPEditor.memo')->caretY;
        c('fmPHPEditor.memo')->insertLine($command);
        c('fmPHPEditor.memo')->caretY = $lineY+1;
    }
		...
		{  
            ...
            if ($edit)
			{
                if (count($action['PARAMS_OBJS']))
				{
                    ...
                    if ( class_exists('myMSBCreator') ) 
						myMSBCreator::generate(c('fmPHPEditor.memo',1)->text);
                }
            }
            else {
                if ( !self::getLine() )
