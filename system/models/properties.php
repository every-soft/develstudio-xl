...
class myProperties 
{
	...
	// Ќова€ ф-и€, тип свойтсв: List
   function createXList(&$param, $class){
		...
		elseif ($param['IS_GUTTER'])
			$edt = _c($this->panels[$class]['GROUP_GUTTER']->add($edt, $param['CAPTION']));
        else
            $edt = _c($this->panels[$class]['GROUP']->add($edt, $param['CAPTION']));
        
        $edt->loadPicture(myImages::get24('Pick'));
        $edt->buttonWidth = 19;
		if(trim($param['REAL_PROP'])>'')
			$edt->hint = $param['CAPTION']." [->{$param['REAL_PROP']}]";
		else
			$edt->hint = $param['CAPTION']." [->{$param['PROP']}]";
        $edt->showHint = true;
        $edt->onButtonClick = 'myProperties::VSListClick';
    ...
    function VSListClick($self){
        ...
        $dlg = new TListDialog();
        $dlg->set_value($myProperties->selObj->$prop , $textto );
    ...
    function VSFontClick($self)
	{
        ...
        $dlg = new TFontDialog;
        // $dlg->font->assign(eval('return $myProperties->selObj->'.$prop.';'));
		$dlg->font->assign($myProperties->getProp($prop));
        
        if ($dlg->execute())
		{
            $font  = $dlg->font;
            $targets = sizeof($_sc->targets_ex) ? $_sc->targets_ex : array($fmEdit);
	...
	function getProp ($prop)
	{
		...
		// костыль чтобы работало.
		return eval('return $myProperties->selObj->'.$prop.';');
	}
    ...
    function VSColorClick($self){
        ...
        $dlg->color = $myProperties->getProp($prop);
    ...
    function VSMenuClick($self){
        ...
        global $myProperties, $toSetProp, $_sc, $fmEdit; //, $fmEdit
        if ($toSetProp) return;
    ...
    function VSButtonClick($self){
        ...
        $dlg = new TTextDialog;
        $dlg->value = $myProperties->getProp($prop);
    ...
    function VSEdit($self, $link, $value, $CAN)
	{
        ...
        if ($param['TYPE'] == 'font')
		{
            ...
            $font = $myProperties->getProp($prop);
        ...
        if ($param['TYPE'] == 'color')
		{
            $color = $myProperties->getProp($prop);
        ...
        } elseif ($param['TYPE']=='combo')
		{
			...
			$value = $param['NO_CONST'] ? array_search($value, $param['VALUES']) : constant($value);
        }
    ...
    function formShow($self, $form, $x = true)
	{
        ...
        if ($info['position'])
		{
			...
			$c_borderstyle->items->selected = 'bsSizeable'; // fix bug
    ...
    function setFontDsgn($obj, $font){
        ...
        $obj->valueFont('pitch', $font->pitch); 
		$obj->valueFont('orientation', $font->orientation); 
		$obj->valueFont('height', $font->height); 
		
        $obj->value = "{$font->name}, {$font->size} px";
    }
    ...
    function _setProps($update = false)
	{
		...
		if(!empty($elements))
		{
			foreach ($elements as $self => $param)
			{
				...
				if ($update && !$param['UPDATE_DSGN']) 
					continue;
				
				$prop = is_array($param['PROP']) ? $param['PROP'][0] : $param['PROP'];
				if ($prop) // fix bug syntax error, unexpected ';', expecting T_STRING or T_VARIABLE or '{' or '$
				...
				} elseif ($param['TYPE']=='font')
				{
					...
					self::setFontDsgn($obj, $this->getProp($prop));
					
				} elseif ($param['TYPE']=='color')
				{
					self::setColorDsgn($obj, $this->getProp($prop));
				...
				} elseif ($param['TYPE'] == 'files') 
				{
					...
					global $projectFile;
					$dir = $param['DIR'] ? $param['DIR'] : dirname($projectFile);
					
					
					$files = findFiles($dir, $param['EXT'], $param['RECURSIVE'], true);
					
					foreach ($files as $file)
					{
						$file = str_replace(array($dir, '//'), array('', '//'), $file);
					...
					if ($obj->value) // fixed!!!
						$this->selObj->$prop = $obj->value;
				
					$obj->value = $this->selObj->$prop;
				
				} else {
					// (string)
					$obj->value = $this->getProp($prop);
				}
        ...
        if (!isset($this->panels[$class]))
		{
            ...
            if (is_array($componentProps[$class]))
			{
				foreach ($componentProps[$class] as &$prop)
				{
					...
					if (!isset($gr3)/* && $prop['IS_GUTTER']*/ && $class == 'TSynEdit')
					{
						$gr3 = new TNxToolbarItem;
						$gr3->caption = t('gr_gutter');
						
						$panel->addItem(null, $gr3);
						
						$this->panels[$class]['GROUP_GUTTER'] = $gr3;
					}
					
					if (!isset($gr2) && $prop['ADD_GROUP'])
					{
    ...
    static function fixSpliterMoved($self){
		if (c('fmPropsAndEvents->tabProps') instanceof DebugClass)
			return;
