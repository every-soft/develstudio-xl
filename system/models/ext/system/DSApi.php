...
class DSApi
{
	...
	static function reg_eventParams ($type, $params, $class = false)
	{
		...
		else $GLOBALS['__EVENTS_API_PRMS'][strtolower ($type)] = $params;
	}
				...
				if (is_array ($eventList))
				{
					...
					foreach ((array) $eventList as $x => $code)
					{
						...
						else
						{
							...
							else $el->$x = $GLOBALS['__EVENTS_API'][$x];
						}
						...
						if ($x == 'oncreate')
						{
							...
							else self::reg_startFunc ('__exEvents::callCode(' . $form->self . ',"oncreate");');
						}
					}
				}
			}
